...
def afficher_dataframe_brut(
    df: pd.DataFrame,
    use_container_width: bool = True
) -> None:
    """
    Affiche un DataFrame de manière identique à Excel, sans aucune transformation.
    
    Règles strictes:
    - Affiche TOUTES les lignes, y compris les vides
    - Ne modifie pas les données
    - N'ajoute ni ne supprime rien
    - Masque uniquement l'index pour correspondre à Excel
    
    Args:
        df: Le DataFrame à afficher
        use_container_width: Si True, utilise toute la largeur disponible
    """
    if df.empty:
        st.warning("Le tableau est vide.")
        return
    
    # Calculer la hauteur pour afficher toutes les lignes
    # Environ 35px par ligne + 40px pour l'en-tête
    row_height = 35
    header_height = 40
    total_height = min(
        1000,  # Hauteur maximale pour éviter les problèmes de performance
    ...
    st.dataframe(
        ...
        hide_index=True  # Seule modification autorisée : masquer l'index
    )
    
    # Afficher le nombre total de lignes pour référence
    st.caption(f"Affichage de {len(df)} lignes (intégralité du tableau)")
...
def assign_codes_from_dict_with_ids(
    ...
    for _, row in codebook_with_ids.iterrows():
        try:
            ...
            code_num = int(row["code"]) if pd.notna(row["code"]) else None
            if code_num is None:
                continue
                
            # Traiter les IDs comme des chaînes de caractères
            ids_list = row["ids"]
            if not isinstance(ids_list, (list, set, tuple)):
                if pd.isna(ids_list):
                    continue
                # Essayer de diviser la chaîne si c'est une chaîne
                if isinstance(ids_list, str):
                    ids_list = [x.strip() for x in re.split(r'[,\s;]+', ids_list) if x.strip()]
                else:
                    ids_list = [str(ids_list).strip()]
            
            # Ajouter les codes pour chaque ID
            for vid in ids_list:
                if not vid or (isinstance(vid, (float, int)) and pd.isna(vid)):
                    continue
                ...
                if vid_str not in id_to_codes:
                    id_to_codes[vid_str] = []
                if code_num not in id_to_codes[vid_str]:  # Éviter les doublons
    ...
    # Déterminer le nombre maximum de codes nécessaires
    if max_codes is None:
        max_codes = max((len(codes) for codes in id_to_codes.values()), default=0)
        if max_codes <= 0:
            max_codes = 1

    # Assigner les codes aux lignes du DataFrame
    assigned: List[List[int]] = []
    for _, row in df.iterrows():
        # S'assurer que l'ID est traité comme une chaîne
        vid = str(row[id_col]).strip() if pd.notna(row[id_col]) else ""
        
        # Récupérer les codes pour cet ID
        codes = list(dict.fromkeys(id_to_codes.get(vid, [])))  # Éviter les doublons
        
        # Si aucun code n'est trouvé, utiliser NA
        if not codes:
            codes = [pd.NA]
        ...
        # Tronquer si nécessaire et ajouter à la liste des codes assignés
...
def _read_sheet_with_header_detection(xls: pd.ExcelFile, sheet_name: str) -> pd.DataFrame:
    """
    Lit une feuille Excel en préservant intégralement son contenu.
    
    Règles strictes :
    - Conserve toutes les lignes, y compris les vides
    - Ne modifie pas les valeurs des cellules
    - Ne supprime aucune ligne ou colonne
    - Ne fait que détecter l'en-tête pour nommer les colonnes
    
    Returns:
        pd.DataFrame: Les données brutes du fichier Excel
    """
    try:
        # Lire le fichier sans aucune transformation
        df = xls.parse(
            sheet_name=sheet_name,
            header=None,  # Ne pas utiliser la première ligne comme en-tête
            dtype=object,  # Préserver les types d'origine
            keep_default_na=False,  # Ne pas convertir les chaînes vides en NaN
            na_filter=False  # Ne pas filtrer les valeurs manquantes
        )
        
        # Si le DataFrame est vide, retourner tel quel
        if df.empty:
            return df
            
        # Détecter la ligne d'en-tête
        header_idx = _guess_header_row(df)
        
        # Extraire les noms de colonnes
        header_values = df.iloc[header_idx].tolist()
        
        # Nettoyer les noms de colonnes (uniquement pour l'affichage)
        ...
        # Créer une copie des données avec les noms de colonnes
        ...
        # En cas d'erreur, essayer la lecture la plus basique possible
        try:
            return pd.read_excel(xls, sheet_name=sheet_name, dtype=object, header=None)
        except Exception:
            # Dernier recours : retourner un DataFrame vide
            return pd.DataFrame()
...
def endpoint_auto_treatment(current: Dict[str, Any], current_idx: int, codebook_parsed: pd.DataFrame, max_codes: int = 1) -> Dict[str, Any]:
    try:
        ...
        # Afficher le tableau exactement comme dans Excel, sans aucune modification
        afficher_dataframe_brut(df_preview)
        ...
        # Convertir les IDs en chaînes de caractères pour éviter les problèmes de type
        if id_col in df.columns:
            df[id_col] = df[id_col].astype(str).str.strip()
        
        # Traiter la colonne de texte
        if txt_col in df.columns:
            df[txt_col] = df[txt_col].astype(str).str.strip()
            mask = (df[txt_col] != "") & (df[txt_col].str.lower() != "none")
