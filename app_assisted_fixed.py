...
# Définition des stopwords français (liste non exhaustive)
_FR_STOP = {
    ...
    'n', 's', 't', 'y', 'été', 'étée', 'étées', 'étés', 'étant', 'étante', 'étants',
    'étantes', 'suis', 'es', 'est', 'sommes', 'êtes', 'sont', 'serai', 'seras', 'sera',
    'serons', 'serez', 'seront', 'serais', 'serait', 'serions', 'seriez', 'seraient',
    ...
    'aviez', 'avaient', 'eut', 'eûmes', 'eûtes', 'eurent', 'aie', 'aies', 'ait',
    'ayons', 'ayez', 'aient', 'eusse', 'eusses', 'eût', 'eussions', 'eussiez',
    'eussent'
}

# Combinaison des stopwords anglais et français
_STOPWORDS = set()
try:
...
# ============================================================================
# FONCTIONS DE LECTURE ET AFFICHAGE FIDÈLE EXCEL
# ============================================================================

def lire_excel_brut(chemin_fichier: Union[str, io.BytesIO], feuille: Union[str, int] = 0) -> pd.DataFrame:
    """
    Lit un fichier Excel sans aucune transformation des données.
    
    Garanties :
    - Aucune conversion de type
    - Aucune inférence d'en-tête
    - Conservation des valeurs vides
    - Aucun filtrage
    
    Args:
        chemin_fichier: Chemin vers le fichier Excel ou objet BytesIO
        feuille: Nom ou index de la feuille à lire
        
    Returns:
        DataFrame brut sans aucune transformation
    """
    try:
        # Lecture brute sans aucune transformation
        df = pd.read_excel(
            chemin_fichier,
            sheet_name=feuille,
            header=None,          # Désactive l'inférence d'en-tête
            dtype=object,         # Désactive la conversion de type
            keep_default_na=False,  # Ne convertit pas les chaînes vides en NaN
            na_filter=False,      # Ne filtre pas les valeurs manquantes
            engine='openpyxl'     # Utilise openpyxl pour une meilleure fidélité
        )
        
        # S'assurer que toutes les colonnes sont traitées comme du texte
        df = df.astype(str)
        
        # Remplacer les 'nan' par des chaînes vides pour la cohérence
        df = df.replace('nan', '')
...
def verifier_fidelite_excel(chemin_fichier: Union[str, io.BytesIO], feuille: Union[str, int] = 0) -> bool:
    """
    Vérifie que la lecture pandas correspond exactement au fichier Excel source.
    
    Cette fonction effectue une vérification complète en comparant cellule par cellule
    entre la lecture avec pandas et la lecture de référence avec openpyxl.
    
    Garanties de fidélité :
    - Valeurs brutes des cellules (sans transformation de type)
    - Ordre des lignes et des colonnes strictement identique
    - Nombre exact de lignes et colonnes préservé
    - Conservation des cellules vides et des valeurs nulles
    - Représentation textuelle identique à l'original
    
    Limites connues :
    - Mise en forme (couleurs, polices, bordures) non préservée
    - Formules Excel non préservées (uniquement les valeurs calculées)
    - Cellules fusionnées désolidarisées
    - Formats de nombres et dates convertis en texte brut
    - Commentaires et validations non conservés
    
    Args:
        chemin_fichier: Chemin vers le fichier Excel ou objet BytesIO
        feuille: Nom ou index de la feuille à vérifier
        
    Returns:
        bool: True si la lecture est fidèle, False en cas d'échec de vérification
        
    Lève:
        ValueError: Si une différence est détectée entre la lecture OpenPyXL et pandas
    """
    try:
        # 1. Lecture avec openpyxl (référence)
        if isinstance(chemin_fichier, io.BytesIO):
            wb = load_workbook(chemin_fichier, read_only=True, data_only=True)
            chemin_fichier.seek(0)
        else:
            wb = load_workbook(chemin_fichier, read_only=True, data_only=True)
            
        ws = wb.worksheets[feuille] if isinstance(feuille, int) else wb[feuille]
        lignes_openpyxl = list(ws.values)
        wb.close()
        
        # 2. Lecture avec notre implémentation
        df = lire_excel_brut(chemin_fichier, feuille)
        if isinstance(chemin_fichier, io.BytesIO):
            chemin_fichier.seek(0)
        
        # 3. Vérification stricte des dimensions
        nb_lignes_source = len(lignes_openpyxl)
        nb_colonnes_source = max((len(ligne) for ligne in lignes_openpyxl), default=0)
        
        if len(df) != nb_lignes_source:
            raise ValueError(
                f"Incohérence détectée : "
                f"{nb_lignes_source} lignes dans le fichier source, "
                f"mais {len(df)} lignes après lecture."
            )
            
        if len(df.columns) < nb_colonnes_source:
            raise ValueError(
                f"Incohérence détectée : "
                f"Jusqu'à {nb_colonnes_source} colonnes dans le fichier source, "
                f"mais seulement {len(df.columns)} colonnes après lecture."
            )
        
        # 4. Vérification cellule par cellule
        for i, ligne in enumerate(lignes_openpyxl, 1):  # 1-based pour les messages d'erreur
            if i-1 >= len(df):  # Sécurité supplémentaire
                raise ValueError(
                    f"Erreur à la ligne {i} : "
                    "Le nombre de lignes ne correspond pas après vérification"
                )
                
            for j, valeur in enumerate(ligne, 1):  # 1-based pour les messages d'erreur
                if j-1 >= len(df.columns):
                    continue  # Ignorer les cellules en dehors du DataFrame
                    
                # Conversion textuelle stricte pour comparaison
                # Note : Cette approche garantit une correspondance exacte de la représentation
                # textuelle, ce qui est plus fiable que la comparaison des types natifs
                # qui pourraient varier selon les versions de pandas/Excel
                val_ref = str(valeur) if valeur is not None else ""
                val_df = str(df.iat[i-1, j-1]) if not pd.isna(df.iat[i-1, j-1]) else ""
                
                if val_ref != val_df:
                    raise ValueError(
                        f"Différence détectée à la cellule {get_column_letter(j)}{i}:\n"
                        f"- Valeur attendue : {repr(val_ref)}\n"
                        f"- Valeur obtenue : {repr(val_df)}"
                    )
        
        return True
        
    except Exception as e:
        st.error(f"❌ Échec de la vérification d'intégrité : {str(e)}")
        st.error("Le fichier ne peut pas être affiché en raison d'incohérences détectées.")
        return False

def afficher_excel_brut(df: pd.DataFrame) -> None:
    """
    Affiche un DataFrame exactement comme dans Excel.
    
    Garanties :
    - Affiche toutes les lignes et colonnes sans exception
    - Conserve les cellules vides
    - Aucune transformation des données
    
    Args:
        df: DataFrame à afficher (doit être le résultat de lire_excel_brut)
    """
    try:
        ...
        # Afficher les dimensions exactes
        nb_lignes = len(df)
        nb_colonnes = len(df.columns)
        
        # Création du tableau HTML avec toutes les lignes et colonnes
        html = "<div style='overflow-x:auto; max-height: 80vh;'>\n"
        html += f"<div style='margin-bottom: 10px; font-size: 0.9em; color: #666;'>"
        html += f"Feuille Excel - {nb_lignes} ligne{'s' if nb_lignes > 1 else ''} × {nb_colonnes} colonne{'s' if nb_colonnes > 1 else ''}"
        html += "</div>\n"
        ...
        for col in range(nb_colonnes):
            html += f"<th>{chr(65 + (col % 26))}{'' if col < 26 else chr(64 + (col // 26))}</th>"
        html += "</tr>\n"
        
        # Données
        for i in range(nb_lignes):
            html += f"<tr><th>{i+1}</th>"  # Numéro de ligne
            for j in range(nb_colonnes):
                # Récupérer la valeur de la cellule
                try:
                    val = str(df.iat[i, j]) if not pd.isna(df.iat[i, j]) else ""
        ...
        # En cas d'erreur, afficher une version simple du tableau
...
def endpoint_auto_treatment(current: Dict[str, Any], current_idx: int, codebook_parsed: pd.DataFrame, max_codes: int = 1) -> Dict[str, Any]:
    try:
        ...
        # Afficher le tableau exactement comme dans Excel
        st.dataframe(
            df_preview,
                height=min(400, 35 * (len(df_preview) + 1)),  # Hauteur dynamique
            width='stretch',
